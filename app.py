...
def save_to_google_sheet(record):
    try:
        ...
        row_data = [
            ...
            record.get("開始時間", ""),
            record.get("終了時間", ""),
            record.get("有線マイク本数", 0),
            record.get("ワイヤレスマイク本数", 0),
            record.get("ピンマイク本数", 0),
            record.get("概算合計金額", 0),
            record.get("概算合計金額(税込)", 0)
        ]
        sheet.append_row(row_data)
        return True, "✅ Googleスプレッドシートにも保存しました！"
    except Exception as e:
        return False, f"⚠️ スプレッドシート保存エラー: {e}"
    ...
    if not matched.empty:
        ...
        with col_res2:
            if st.button("💾 この見積を履歴に保存", use_container_width=True):
                ...
                # Googleスプレッドシート保存
                success, msg = save_to_google_sheet(record)
        ...
        with st.expander("🖨️ 印刷 / PDF保存用のプレビューを表示", expanded=False):
            ...
            <head>
                ...
                <style>
                    body {{ font-family: "Helvetica Neue", Arial, "Hiragino Kaku Gothic ProN", "Hiragino Sans", Meiryo, sans-serif; padding: 15px; color: #333; }}
                    #pdf-area {{ padding: 10px; background: #fff; width: 100%; box-sizing: border-box; }}
                    h2 {{ border-bottom: 2px solid #333; padding-bottom: 5px; margin-top: 0; font-size: 18px; }}
                    ...
                    table.print-table th, table.print-table td {{ border: 1px solid #ccc; padding: 6px 8px; text-align: left; font-size: 11px; white-space: normal; word-break: break-all; }}
